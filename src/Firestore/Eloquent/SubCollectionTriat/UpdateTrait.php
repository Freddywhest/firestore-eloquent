...
trait UpdateTrait
{
     ...
     * @throws \Exception
     */
    protected function fUpdate(array $data, $firestore, $documentId, $primaryKey, $fillable, $required, $fieldTypes, $subCollectionName)
    {
        if(!$documentId){
            throw new \Exception('Invalid document ID or ID does not exist in the "'.$subCollectionName.'" collection.', 1);
        }
        ...
        if(count($filteredData) > 0){
            try {
                $firestore->document($documentId)->update($filteredData);
            } catch (\Exception $e) {
                throw new \Exception('Error updating document: '.$e->getMessage(), 1);
            }
        }

        throw new \Exception('No data to update because document does not exist.', 1);
    }
}
